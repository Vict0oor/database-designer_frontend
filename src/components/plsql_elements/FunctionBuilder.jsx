...
const FunctionBuilder = ({ onJsonUpdate, databaseTables }) => {
    ...
    const [returnType, setReturnType] = useState(ATTRIBUTE_TYPES[0]);
    const [returnSize, setReturnSize] = useState('');
    const [returnPrecision, setReturnPrecision] = useState('');

    const paramTypes = ['IN'];

    const stepTypes = [
        'QUERY', 'SELECT INTO', 'IF-ELSE', 'LOOP', 'EXCEPTION', 'RETURN', 'CUSTOM'
    ];
        ...
        return stepsArray.map((step, index) => {
            ...
            return (
                <div key={currentPath.join('-')} className="p-3 mb-3 bg-black rounded border">
                    ...
                    {step.type === 'RETURN' && (
                        <div className="mb-2">
                            <label className="block mb-1 text-sm">Return value:</label>
                            <input
                                type="text"
                                value={step.content || ''}
    ...
    useEffect(() => {
        const generateJson = () => {
            const functionData = { 
                ...
                returnType: {
                    type: returnType,
                    size: supportsLength(returnType) ? returnSize || null : null,
                    precision: supportsPrecision(returnType) ? returnPrecision || null : null
                },
            ...
            onJsonUpdate(functionData, "FUNCTION");
        };

        generateJson();
    }, [functionName, returnType, returnSize, returnPrecision, parameters, variables, steps]);

    return (
        <div className="bg-black p-6 rounded-lg shadow-lg">
            <h1 className="text-2xl font-bold mb-6 text-center">Function Builder PL/SQL</h1>

            <div className="mb-6">
                <label className="block mb-2 font-medium text-xl">Function name:</label>
                <input
                    type="text"
                    value={functionName}
            ...
            <div className="mb-6">
                ...
                <div className="flex flex-wrap items-center gap-2 p-3 bg-black rounded border">
                    ...
                    {supportsLength(returnType) && (
                        <div className="w-20">
                            ...
                            <input
                                ...
                                step="1"
                                value={returnSize}
                                onChange={(e) => setReturnSize(e.target.value === '' ? '' : Math.max(0, parseInt(e.target.value)))}
                                className="w-full p-1 border rounded"
                                placeholder="Length"
                            />
                        </div>
                    ...
                    {supportsPrecision(returnType) && (
                        <div className="w-20">
                            ...
                            <input
                                ...
                                onChange={(e) => setReturnPrecision(e.target.value === '' ? '' : Math.max(0, parseInt(e.target.value)))}
                                className="w-full p-1 border rounded"
            ...
            <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                    <h2 className="text-xl font-semibold">Function Steps</h2>
                    <button
                        onClick={() => addStep()}
                        className="bg-orange-500 cursor-pointer text-white px-3 py-1 rounded hover:bg-orange-400"
                    >
